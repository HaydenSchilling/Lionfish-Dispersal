...
file_list <- list.files(path = "/srv/scratch/z3374139/Lionfish/Forward/From 2024/", pattern = ".nc", full.names = TRUE)
...
data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2024/Full_paths.csv")

full_dat <- full_dat %>% filter(age == 26)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2024/Full_end_points.csv")

full_dat <- full_dat %>% filter(bathy <= 350 & bathy > 0)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2024/Shallow_end_points.csv")

full_dat <- full_dat %>% sample_n(size=1000, replace = F)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2024/Shallow_end_points_reduced.csv")
