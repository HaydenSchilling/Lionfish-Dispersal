...
file_list <- list.files(path = "/srv/scratch/z3374139/Lionfish/Forward/From 2029/", pattern = ".nc", full.names = TRUE)
...
write_csv(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2029/Full_paths.csv")

full_dat <- full_dat %>% filter(age == 26)

write_csv(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2029/Full_end_points.csv")

full_dat <- full_dat %>% filter(bathy <= 350 & bathy > 0)

write_csv(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2029/Shallow_end_points.csv")

full_dat <- full_dat %>% sample_n(size=1000, replace = F)

write_csv(full_dat, "/srv/scratch/z3374139/Lionfish/Forward/From 2029/Shallow_end_points_reduced.csv")
