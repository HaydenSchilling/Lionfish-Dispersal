...
file_list <- list.files(path = "/srv/scratch/z3374139/Lionfish/2018/", pattern = ".nc", full.names = TRUE)
...
data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2018/Full_paths.csv")

full_dat <- full_dat %>% filter(age == 26)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2018/Full_end_points.csv")

full_dat <- full_dat %>% filter(bathy <= 350 & bathy > 0)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2018/Shallow_end_points.csv")

full_dat <- full_dat %>% slice_min(lon, n=1000)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2018/Shallow_end_points_reduced.csv")
