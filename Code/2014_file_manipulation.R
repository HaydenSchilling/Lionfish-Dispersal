...
file_list <- list.files(path = "/srv/scratch/z3374139/Lionfish/2014/", pattern = ".nc", full.names = TRUE)
...
data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2014/Full_paths.csv")

full_dat <- full_dat %>% filter(age == 26)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2014/Full_end_points.csv")

full_dat <- full_dat %>% filter(bathy <= 350)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2014/Shallow_end_points.csv")

full_dat <- full_dat %>% slice_min(lon, n=1000)

data.table::fwrite(full_dat, "/srv/scratch/z3374139/Lionfish/2014/Shallow_end_points_reduced.csv")
